...
**`.readnext` — Continue Reading.** Three curated article rows plus a `.readnext-all` link to `/resources/`. A body-level sibling of `.page-wrapper`, positioned after `.qcard-wrap`, never a grid child. Rows are `display: grid; grid-template-columns: 152px 1fr` — a mono uppercase tag column beside title and description — collapsing to a single column at `max-width: 700px`, where the tag stacks above the title. The tag, title, and description on every row are **copied verbatim from `resources/index.html`**, so the two surfaces never drift; do not retype or reword them. **One exception:** a page may tailor a row's title and description to its own article when the tailored copy explains why that paper is the next read *from this page* — the tag and `href` still match the index exactly. `hud-multifamily-mip` does this deliberately (2026-09-17): its three descriptions tie each paper to MIP, and its 241(a) title is shortened. An audit comparing rows against the index should expect these differences on that page and not flag them. Any other page drifting from the index is still a defect. Recommendations are an editorial choice, not keyword similarity — avoid the failure mode where every page points at the same popular paper, and never let a page recommend itself. All 22 resource pages carry exactly one `.readnext`, 66 rows in total.
...
**So a mobile audit must measure element widths against the viewport, not look for visible overflow.** Compare each candidate's `getBoundingClientRect().width` and `.right` to `document.documentElement.clientWidth` (**not** `innerWidth` — see below); checking `document.documentElement.scrollWidth > clientWidth` will report no problem on a page that has one.

**`window.innerWidth` is the wrong basis for that comparison, and it fails in exactly the wrong direction.** On a page that actually overflows, `innerWidth` tracks the expanded layout width rather than the viewport, so comparing against it reports no overflow on precisely the pages that have it. Found 2026-08-28 on `hud-interest-only-vs-amortizing`, where a resize to 375px reported `innerWidth` **643** while `documentElement.clientWidth` was **375** and `scrollWidth` was **643** — 268px of real overflow (a `table.cmp` with `min-width: 420px` and no scroll wrapper) that an `innerWidth` comparison called clean. Always compare element rects and `scrollWidth` against `documentElement.clientWidth`.
